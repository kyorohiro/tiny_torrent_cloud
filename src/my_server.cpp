...
void file_manager(std::string base_url_path, std::string base_file_path,
...
void _static_file_handle(std::string filepath, const httplib::Request &req,
                         httplib::Response &res) try {
...
void listen(std::string ip, int port, std::string username,
            std::string password, std::string target_file_path,
  ...
  _http_server.Get("/create_magnetlink", [](const httplib::Request &req,
                                            httplib::Response &res) {
    std::cout << "[request]: "
              << "/create_magnetlink" << std::endl;
    _static_file_handle("./res/html/create_magnetlink.html", req, res);
  });
  _http_server.Get("/add_seed_files", [](const httplib::Request &req,
                                         httplib::Response &res) {
    std::cout << "[request]: "
              << "/seed_files" << std::endl;
    _static_file_handle("./res/html/seed_files.html", req, res);
  });

  _http_server.Get(
      "/magnetlink", [](const httplib::Request &req, httplib::Response &res) {
        std::cout << "[request]: "
                  << "/magnetlink" << std::endl;
        _static_file_handle("./res/html/magnetlink.html", req, res);
      });

  _http_server.Get("/ip_check",
                   [](const httplib::Request &req, httplib::Response &res) {
                     std::cout << "[request]: "
                               << "/ip_check" << std::endl;
  ...
  _http_server.Get("/history_ip_info", [](const httplib::Request &req,
                                          httplib::Response &res) {
    std::cout << "[request]: "
  ...
  // api other
  //
  _http_server.Post("/api/get_info_from_ip", [](const httplib::Request &req,
                                                httplib::Response &res) {
    ...
    try {
      ...
      nlohmann::json o;
      o["country"] = my_ip_country_detector::find_country_from_ip(
          inp["ip"].get<std::string>());
      o["domain"] = my_ip_country_detector::find_dns_from_ip(
          inp["ip"].get<std::string>());
  ...
  _http_server.Post("/api/history_ip/list", [](const httplib::Request &req,
                                               httplib::Response &res) {
  ...
  // api magnetlink
  //
  _http_server.Post("/api/magnetlink/add", [](const httplib::Request &req,
                                              httplib::Response &res) {
    ...
    try {
      ...
      nlohmann::json inp = nlohmann::json::parse(b);
      nlohmann::json o;
      my_db::TargetInfo target_info =
          my_db::insert_magnetlink(inp["magnetlink"].get<std::string>());
      my_torrent::add_magnetlink(target_info.unique_id,
                                 inp["magnetlink"].get<std::string>());
      res.set_content(o.dump(), "text/json");
      res.status = 200;
    } catch (...) {
  ...
  _http_server.Post("/api/torrentfile/add", [](const httplib::Request &req,
                                               httplib::Response &res) {
    ...
    try {
      ...
      std::cerr << e.what() << req.body << std::endl;
      std::cerr << "error at /api/torrentfile/add" << req.body << std::endl;
      res.status = 501;
    }
  });

  _http_server.Post("/api/magnetlink/remove", [](const httplib::Request &req,
                                                 httplib::Response &res) {
  ...
  _http_server.Post("/api/magnetlink/create", [](const httplib::Request &req,
                                                 httplib::Response &res) {
  ...
  _http_server.Post("/api/magnetlink/list", [](const httplib::Request &req,
                                               httplib::Response &res) {
  ...
  _http_server.Post("/api/seed_file", [](const httplib::Request &req,
                                         httplib::Response &res) {
    std::cout << "[request]: /api/seed_file" << std::endl;

    try {
      if (!handleAuthCheck(req, res)) {
        return;
      }
      httplib::MultipartFormData seed_file = req.get_file_value("seed_file");

      const boost::uuids::uuid id1 = boost::uuids::random_generator()();
      std::string uid = boost::lexical_cast<std::string>(id1);
      std::cout << "filename : " << seed_file.filename << std::endl;
      // std::string dataPath = "./.data/" + uid + ".dat";
      std::string tmpPath =
          my_torrent::_downloaded_file_path + "/" + uid + ".dat";
      // std::string torrentPath = "./data/" + uid + ".torrent";
      {
        std::ofstream ofs(tmpPath, std::ios::binary);
        ofs << seed_file.content;
      }
      //
      std::cout << "[request]: /api/seed_file 1 gen torrent" << std::endl;
      std::vector<char> torrentfile_source;
      auto torrent_info = my_torrent::gen_torrentfile(
          std::back_inserter(torrentfile_source), tmpPath);
      //
      // save
      std::cout << "[request]: /api/seed_file 1 save torrent" << std::endl;
      //

      // std::string s(torrentfile_source.begin(), torrentfile_source.end());
      // std::cout << torrentfile_source.data() << std::endl;

      const char *torrentfile_source_cstr = torrentfile_source.data();
      my_db::TargetInfo target_info = my_db::save_torrent_file(
          torrentfile_source_cstr, torrentfile_source.size());
      my_torrent::add_torrentfile(target_info.unique_id, target_info.target);

      // 保存
      // lt::torrent_info xx((const char *)torrentfile_source_cstr,
      //                   (int)torrentfile_source.size());
      ...
      //
      //
      std::cout << "[request]: /api/seed_file 1 resp" << std::endl;
      nlohmann::json o;
      o["filename"] = seed_file.filename;
      o["length"] = seed_file.content.length();
      res.set_content(o.dump(), "text/json");
      res.status = 200;
    } catch (std::exception &e) {
      std::cerr << e.what() << std::endl;
      std::cerr << "error at /api/seed_file" << req.body << std::endl;
      res.status = 501;
    }
  });
...
void file_manager(std::string base_url_path, std::string base_file_path,
  ...
  if (fs::is_regular_file(p)) {
    ...
    for (; itr != end_itr; ++itr) {
      ...
      ss << "<a href=\""
         << "/" + base_url_path + "/"
         << my_base_encode::encode_url(
                path + (path.size() == 0 ? "" : "/") + filename, false, true)
         << "\">" << filename << "</a><br>";
    }
