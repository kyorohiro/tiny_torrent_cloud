...
void start_http_server_on_thread(int unused) {
  my_server::listen(http_server_ip, http_server_port, username, password,
                    store_folder_path, downloaded_folder_path);
}

void start_torrent_client_on_thread(int unused) { my_torrent::listen(); }

int main(int argc, char *argv[]) {

  // load a pair informaton about ip and contry
  my_ip_country_detector::setup_context(setting_ipv4_cvs_path,
                                        setting_ipv6_cvs_path);
  ...
  for (auto l : target_info_list) {
    if (std::regex_match(l->target, re)) {
      //
      {
        ...
        my_torrent::add_magnetlink(l->unique_id,
