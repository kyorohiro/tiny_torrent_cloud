...
//
bool _put_ip(std::string key, std::string ip, int port, std::string type,
...
struct my_peer_plugin : lt::peer_plugin {
  ...
  void _set_connected_to_db() {
    // std::cout << "DEBUG(1)"<<_name<<","<< _ip<<","<< _port<<","<< "C"<<","<<
    // _key <<std::endl;
    if (!_connectFlagToDB) {
      // std::cout << "DEBUG(2)"<<_name<<","<< _ip<<","<< _port<<","<<
      // "C"<<","<< _key <<std::endl;
  ...
  bool on_have(lt::piece_index_t i) override {
    std::cout << "[plugin] on_have " << _key << ":" << _ip << ":" << _port
              << " on_have " << i << std::endl;
    _set_have_to_db();
    return false;
  }

  bool on_dont_have(lt::piece_index_t i) override { return false; }
  bool on_bitfield(lt::bitfield const &bitfield) override {
    std::cout << "[plugin] on_bitfield " << _key << ":" << _ip << ":" << _port
              << " on_bitfield " << bitfield.count() << "/" << bitfield.size()
              << std::endl;
    if (bitfield.count() > 0) {
      _set_have_to_db();
    }
  ...
  bool on_have_all() override {
    std::cout << "[plugin] on_have_all " << _key << ":" << _ip << ":" << _port
              << " on_have_all " << std::endl;
    _set_have_to_db();
    return false;
  }
  bool on_have_none() override { return false; }
  void on_disconnect(lt::error_code const &e) override {}

  void on_connected() override {
    std::cout << "[plugin] on_connected " << _key << ":" << _ip << ":" << _port
              << " on_connect " << std::endl;
...
struct my_torrent_plugin : lt::torrent_plugin {
  std::string _key;
  std::string _name;
  my_torrent_plugin(std::string key) { _key = key; }

  std::shared_ptr<lt::peer_plugin>
  new_connection(lt::peer_connection_handle const &pch) override {
    std::cout << "[plugin][new_connecton]" << pch.remote().address().to_string()
              << ":" << pch.local_endpoint().port() << std::endl;
    return std::make_shared<my_peer_plugin>(_key, _name,
                                            pch.remote().address().to_string(),
                                            pch.local_endpoint().port());
  }

  void on_add_peer(lt::tcp::endpoint const &endpoint,
                   lt::peer_source_flags_t flag1,
                   lt::add_peer_flags_t flag2) override {
    ...
    //
    // found ip
    std::cout << "[plugin][on_add_peer][" << type << "]"
              << endpoint.address().to_string() << ":" << endpoint.port()
              << std::endl;
    auto h = _torrent_handle_map[_key];
    _name = h.status().name;

    _put_ip(h.status().name, endpoint.address().to_string(), endpoint.port(),
            type, _key);
    // h.name();
  }
};
...
struct plugin_creator {
  std::string _key;
  plugin_creator(std::string key) { _key = key; }

  std::shared_ptr<lt::torrent_plugin> operator()(lt::torrent_handle const &h,
                                                 lt::client_data_t) {
...
bool _put_ip(std::string key, std::string ip, int port, std::string type,
  ...
  if (!alreadyHas) {
    ...
    if (!my_db::alreadtExist(ip, port, type)) {
      my_db::insert_found_ip(key, ip, port, ip_info->country, ip_info->domain,
                             time(nullptr), "", type, unique_id);
    }
  }
...
void setup(std::string bind_address, int upload_max, int download_max,
  ...
  try {
    boost::filesystem::create_directories(_downloaded_file_path);
  } catch (...) {
  }
  //
  // create session
  ...
  session_params.set_int(lt::settings_pack::alert_mask,
...
void add_magnetlink(std::string key, std::string magnetlink) {
  auto magnetlink_info = lt::parse_magnet_uri(magnetlink);
  lt::add_torrent_params torrent_params;

  torrent_params.save_path =
      my_torrent::_downloaded_file_path + "/" + key + "/"; // save in this dir
  lt::error_code ec;
...
void add_torrentfile(std::string key, std::string path) {
  lt::add_torrent_params torrent_params;
  torrent_params.save_path =
      my_torrent::_downloaded_file_path + "/" + key + "/";
  // _downloaded_file_path; //".data"; // save in this dir
  lt::error_code ec;
...
void listen() {
  while (_interrupted == false) {
    ...
    for (lt::alert *a : alerts) {
      // log for develop
      std::cout << "[" << a->type() << "](" << a->what() << ") " << a->message()
                << std::endl;
    }
    if (0 == alerts.size()) {
